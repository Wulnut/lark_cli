---
title: "获取指定的工作项列表（单空间-复杂传参） - 开发者手册 - 飞书项目帮助中心"
source: "https://project.feishu.cn/b/helpcenter/1p8d7djs/19pti4e2"
author:
published:
created: 2026-03-21
updated: 2026-03-21
description: "在指定空间内使用复杂筛选条件搜索工作项实例，支持 SearchGroup 结构"
tags:
  - "飞书项目"
  - "工作项"
  - "获取工作项列表"
...
related_docs:
  ...
  - "[[获取指定的工作项列表（全量搜索） - 开发者手册 - 飞书项目帮助中心]]"
---
...
| work_item_type_key | string | 必须   | 工作项类型，系统自动生成，可通过 [获取空间下工作项类型](https://project.feishu.cn/b/helpcenter/1p8d7djs/3pjp854w) 接口获取                                                                                                                     |

Body

| 参数 | 类型 | 是否必须 | 说明 |
| --- | --- | --- | --- |
| search_group | object | 必须 | 筛选条件（支持上限最多50个），用于组合条件搜索时使用。遵循 [SearchGroup](https://project.feishu.cn/b/helpcenter/1p8d7djs/1x1d372l#a889b622) 结构规范，可参考 [搜索参数格式及常用示例](https://project.feishu.cn/b/helpcenter/1p8d7djs/1l8il0l6) 。 |
| page_size | int64 | 可选 | 每页返回的数据条数，最大值为 50，默认按照工作项 ID 排序。   分页设置不能控制总返回数量，查询结果超过 5000 会报错。 |
| page_num | int64 | 可选 | 分页页码，从1开始，默认为1 |
| fields | list<string> | 可选 | 定义需要返回的字段，非必填，默认返回全部。字段格式可查看 [字段与属性解析格式](https://project.feishu.cn/b/helpcenter/1p8d7djs/1tj6ggll) 。支持两种模式，两种模式不可混用。  - 指定字段模式：仅返回节点表单 fields 中列出的字段。如 \["name", "created_at"\]，表示只返回 name 和 created_at 字段及相关信息。 - 排除字段模式：以 - 开头的字段表示排除该字段。如 \["-name", "-created_at"\]，表示返回除 name 和 created_at 外的所有字段及相关信息。 |
...
| 20003 | Wrong WorkItemType Param | 20003 | need_workflow | - 获取指定的工作项列表：work_item_type_keys未填 - 获取工作项详情/获取指定的工作项列表等接口，目标工作项为状态流，但是设置了"expand"参数:{ "need_workflow":true} |
