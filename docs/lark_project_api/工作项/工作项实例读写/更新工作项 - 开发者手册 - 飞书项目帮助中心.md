---
title: "更新工作项 - 开发者手册 - 飞书项目帮助中心"
source: "https://project.feishu.cn/b/helpcenter/1p8d7djs/3e73r2k5"
author:
published:
created: 2026-03-21
updated: 2026-03-22
description: "修改指定空间和工作项类型下的一个工作项实例字段信息"
tags:
  - "飞书项目"
  - "工作项"
  - "工作项实例读写"
...
| 参数 | 类型 | 是否必须 | 说明 |
| --- | --- | --- | --- |
| update_fields | list&lt;object&gt; | 必须 | 目标更新的字段列表，遵循 [FieldValuePair](https://project.feishu.cn/b/helpcenter/1p8d7djs/1x1d372l#10c42182) 结构规范。对指定的字段更新，更新后的值会覆盖之前的值。不支持模版、投票字段修改。 |
...
| 1000051743 | Can not find user info | 1000051743 | Can not find user info | 用户无效，可能原因为 Header 中传入的 X-USER-KEY 不正确，user_key 在飞书项目空间双击用户头像获取。 |
| 10211 | Token Info Is Invalid | 10211 | token is nil | 使用的 Token 不正确或者已过期，请重新获取。 |

## 注意事项

1. 当请求参数中存在级联单选或级联多选字段时，传入的选项值需满足空间配置中该字段的"可选层级"配置。

2. 写接口中选项类型字段的 field_value 应传入的是选项 id（value），而非选项的名称（label）。

3. 角色和人员字段（role_owners）：
   - 流程角色配置的"允许用户自行增删"未勾选时，使用创建和更新接口时，未传入该字段会自动计算角色和成员的默认值。
   - 流程角色配置的"允许用户自行增删"勾选时，使用创建和更新接口时，未传入该字段不会自动计算角色与成员的默认值。
